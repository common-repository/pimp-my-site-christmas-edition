...
if ( ! class_exists( 'Pimp_My_Site_Settings' ) ) :
	...
	class Pimp_My_Site_Settings {
		...
		public function __construct() {
			...
			$this->settings = array(
				...
				"{$this->option_group}_preset_options_section" => array(
					...
					'title'  => sprintf( esc_html__( '%1$s Preset Options', 'pimp-my-site' ), ! empty( $selected_preset ) && is_array( $selected_preset ) ? $selected_preset['label'] : '' ),
					'fields' => array(),
				),
				"{$this->option_group}_stickers_section"       => array(
					'title'  => esc_html__( 'Stickers', 'pimp-my-site' ) . '<a href="https://pimp-my.site" target="_blank" class="button button-small pimp-my-site-meta-box__unlock-button"><span class="dashicons dashicons-awards"></span> ' . esc_html__( 'Unlock all customization options with full version' ) . '</a>',
					'fields' => array(
				...
				"{$this->option_group}_cursor_section"         => array(
					'title'  => esc_html__( 'Cursor', 'pimp-my-site' ) . '<a href="https://pimp-my.site" target="_blank" class="button button-small pimp-my-site-meta-box__unlock-button"><span class="dashicons dashicons-awards"></span> ' . esc_html__( 'Unlock all customization options with full version' ) . '</a>',
					'fields' => array(
				...
				"{$this->option_group}_scrollbar_section"      => array(
					'title'  => esc_html__( 'Scrollbar', 'pimp-my-site' ) . '<a href="https://pimp-my.site" target="_blank" class="button button-small pimp-my-site-meta-box__unlock-button"><span class="dashicons dashicons-awards"></span> ' . esc_html__( 'Unlock all customization options with full version' ) . '</a>',
					'fields' => array(
				...
				"{$this->option_group}_particles_section"      => array(
					'title'  => esc_html__( 'Particles', 'pimp-my-site' ) . '<a href="https://pimp-my.site" target="_blank" class="button button-small pimp-my-site-meta-box__unlock-button"><span class="dashicons dashicons-awards"></span> ' . esc_html__( 'Unlock all customization options with full version' ) . '</a>',
					'fields' => array(
				...
				<ul class="pimp-my-site-radio-image-control">
					<li>
						...
						<label for="<?php echo esc_attr( $args['id'] . 'none' ); ?>">
							<img loading="lazy" src="<?php echo esc_url( PIMP_MY_SITE_PLUGIN_DIR_URL . 'assets/images/icon-none.svg' ); ?>">
						</label>
					</li>
				</ul>
			...
			<div class="pimp-my-site-selector-field pimp-my-site-selector-field--<?php echo esc_attr( $args['id'] ) ?>">
				...
				<div class="pimp-my-site-selector-field__selected-option">
					<?php if ( ! empty( $selected ) ) : ?>
						<img loading="lazy" src="<?php echo esc_url( $selected_image ); ?>">
					<?php else : ?>
						<img loading="lazy" src="<?php echo esc_url( PIMP_MY_SITE_PLUGIN_DIR_URL . 'assets/images/icon-none.svg' ); ?>">
					<?php endif; ?>
				</div>
									...
									<div class="modal__sidebar">
										...
										<div class="modal__sidebar-content">
											...
											<ul class="pimp-my-site-selector-field__filters">
												...
												<?php foreach ( $args['options'] as $category ) : ?>
													<li><a href="#<?php echo esc_attr( $args['id'] . '-' . $category['label'] ); ?>"><?php echo esc_html( $category['label'] ); ?></a></li>
												<?php endforeach; ?>
											</ul>
		...
		public function process_preset() {
			if ( ! isset( $_POST["{$this->option_group}_preset"] ) ) {
				return;
			}

			if ( ! empty( $preset = sanitize_text_field( $_POST["{$this->option_group}_preset"] ) ) && check_admin_referer( 'pimp_my_site_apply_preset' ) ) {
		...
		public function add_option_page() {
			add_options_page(
				esc_html__( 'Pimp my Site - Christmas Edition Settings', 'pimp-my-site' ),
				esc_html__( 'Pimp my Site - Christmas Edition', 'pimp-my-site' ),
				'manage_options',
				$this->page_slug,
		...
		public function get_option( $option_name ) {
			...
			if ( isset( $setting_args['presets'] ) && ! in_array( $selected_preset_name, $setting_args['presets'] ) ) {
				return false;
			}

			if ( ! empty( $selected_preset ) && isset( $selected_preset['options'][ $option_name ] ) ) {
				return $selected_preset['options'][ $option_name ];
			}
		...
		public function render_settings_page() {
			...
			<div class="wrap pimp-my-site-wrap">
				...
				<div class="pimp-my-site-headerbar">
					...
					<strong class="pimp-my-site-headerbar__title"><?php esc_html_e( 'Pimp my Site - Christmas Edition', 'pimp-my-site' ); ?> 🎅</strong>

					<div class="pimp-my-site-headerbar__actions">
						<a href="https://pimp-my.site" target="_blank" class="pimp-my-site-btn pimp-my-site-btn--secondary"><span class="dashicons dashicons-awards"></span> <?php esc_html_e( 'Discover Full Version', 'pimp-my-site' ); ?></a>
					</div>
				...
				<div class="pimp-my-site-settings-container">
												...
												<div class="modal__sidebar-content">
													...
													<ul class="pimp-my-site-presets-modal__filters">
														...
														<?php foreach ( $this->presets as $category_name => $category ) : ?>
															<li>
																<a href="#<?php echo esc_attr( 'presets-' . $category_name ); ?>">
																	<?php echo esc_html( $category['label'] ); ?>
																	<?php if ( $category_name !== 'christmas' ) : ?>
													...
													<?php foreach ( $this->presets as $category_name => $category ) : ?>
														...
														<div class="pimp-my-site-options-category" id="<?php echo esc_attr( 'presets-' . $category_name ); ?>">
															<h4>
																<?php echo esc_html( $category['label'] ); ?>
																<?php if ( $category_name !== 'christmas' ) : ?>
																	<a href="https://pimp-my.site" class="button button-small pimp-my-site-meta-box__unlock-button"><span class="dashicons dashicons-awards"></span> <?php esc_html_e( 'Unlock all presets with full version', 'pimp-my-site' ); ?></a>
																<?php endif; ?>
															</h4>
															...
															<ul class="pimp-my-site-presets-control">
																<?php foreach ( $category['options'] as $value => $preset ) : ?>
																	<li class="pimp-my-site-preset <?php echo ( $value === $selected_preset_name ) ? esc_attr( 'pimp-my-site-selected-preset' ) : ''; ?>">
																		...
																		<?php else : ?>
																			<div class="pimp-my-site-preset__unlock-button">
																				<a href="https://pimp-my.site" class="button button-primary"><span class="dashicons dashicons-awards"></span> <?php esc_html_e( 'Unlock with full version', 'pimp-my-site' ); ?></a>
																			</div>
																		<?php endif; ?>
					...
					<div class="pimp-my-site-settings-sidebar">
						...
						<div class="themosaurus-promo">
							<h2><?php esc_html_e( 'Want to unleash the full potential of Pimp my Site?', 'pimp-my-site' ); ?></h2>
							<h3><?php esc_html_e( 'Unlock all presets for each holiday and special occasion, and get full control over stickers, cursors, scrollbar and particles customization!', 'pimp-my-site' ); ?></h3>
							<div class="pimp-my-site-full">
								<a target="_blank" href="https://pimp-my.site/">
									<img loading="lazy" src="https://files.themosaurus.com/bp-verified-member/pimp-my-site-promo.png" alt="Pimp my Site Promo Image">
								</a>
							</div>
						</div>
